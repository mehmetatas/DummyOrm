...
namespace DummyOrm2.Orm.Sql
{
    ...
    {
        SqlCommand Build<T>(SelectQuery<T> query);
    }

    public class SelectSqlCommandBuilderImpl : ISelectSqlCommandBuilder
    {
        public SqlCommand Build<T>(SelectQuery<T> query)
        {
            var cmd = new StringBuilder();
            var param = new Dictionary<string, SqlParameter>();

            cmd.AppendLine("SELECT")
                .AppendLine(String.Join(",\n", query.Columns.Values.Select(c => String.Format("  {0}.{1} {2}", c.Table.Alias, c.Meta.ColumnName, c.Alias))))
